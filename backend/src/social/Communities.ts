...
export interface CommunityConfig {
  ...
  signatureSimilarityThreshold: number;
  decayRate: number;
}
  ...
  private mergeSimilarCommunities(): void {
    ...
    for (let i = 0; i < communities.length; i++) {
      ...
      for (let j = i + 1; j < communities.length; j++) {
        ...
        for (let c = 0; c < 4; c++) {
          similarity +=
            1 - Math.abs(c1.dominantSignature[c] - c2.dominantSignature[c]);
        }
        similarity /= 4;
    ...
    return {
      ...
      avgAge:
        communities.length > 0
          ...
          : 0,
      largest: communities.reduce(
        (max, c) => (c.population > (max?.population || 0) ? c : max),
