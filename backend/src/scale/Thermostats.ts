...
export type ThermostatType = 
  | 'population'      // Mantiene población en rango
  | 'resources'       // Evita agotamiento total
  | 'energy'          // Balance energético global
  | 'tension'         // Previene conflictos extremos
  | 'diversity'       // Mantiene diversidad genética
  | 'activity';       // Evita estancamiento
...
export interface ThermostatConfig {
  ...
  // PID gains
  kP: number;          // Proportional
  kI: number;          // Integral
  kD: number;          // Derivative
  
  // Limits
  integralMax: number; // Anti-windup
  outputMin: number;
  outputMax: number;
  
  // Timing
  sampleRate: number;  // Ticks entre muestras
}
...
const DEFAULT_CONFIGS: Record<ThermostatType, Partial<ThermostatConfig>> = {
  ...
  tension: {
    target: 0.3,        // Tensión saludable (algo de conflicto)
    min: 0.0,
    max: 0.8,           // Evitar guerras totales
    kP: 0.3,
    kI: 0.03,
    kD: 0.15
  },
  diversity: {
    target: 0.7,        // Alta diversidad genética
    min: 0.3,
    max: 1.0,
    kP: 0.1,
    kI: 0.005,
  ...
  activity: {
    target: 0.5,        // Movimiento promedio
    min: 0.1,
    max: 1.0,
    kP: 0.2,
    kI: 0.02,
...
export class Thermostat {
  ...
  update(currentValue: number): number {
    ...
    this.integral += error;
    this.integral = Math.max(-this.config.integralMax, 
    ...
    this.output = Math.max(this.config.outputMin, 
  ...
  getReading(): ThermostatReading {
    const derivative = this.history.length >= 2
   ...
   */
  isHealthy(): boolean {
    return this.lastValue >= this.config.min && this.lastValue <= this.config.max;
  }
  ...
  isCritical(): boolean {
    ...
    const criticalThreshold = range * 0.1; // 10% de los bordes
    
    return this.lastValue < this.config.min + criticalThreshold ||
  ...
  setTarget(newTarget: number): void {
    this.config.target = Math.max(this.config.min, 
                                 Math.min(this.config.max, newTarget));
    // Reset integral para evitar overshoots
    this.integral = 0;
  }
...
export class ThermostatBank {
  ...
  constructor(configs?: Partial<Record<ThermostatType, Partial<ThermostatConfig>>>) {
    // Crear termostatos por defecto
    const types: ThermostatType[] = [
      'population', 'resources', 'energy', 'tension', 'diversity', 'activity'
    ];
   ...
   */
  updateAll(values: Partial<Record<ThermostatType, number>>): Record<ThermostatType, number> {
   ...
   */
  getHealthStatus(): { healthy: ThermostatType[]; unhealthy: ThermostatType[]; critical: ThermostatType[] } {
   ...
   */
  getSuggestedActions(): Array<{ type: ThermostatType; action: string; urgency: number }> {
    const actions: Array<{ type: ThermostatType; action: string; urgency: number }> = [];
   ...
   * Stats resumidos
   */
  getStats(): Record<ThermostatType, { output: number; trend: string; healthy: boolean }> {
    const stats: Record<string, { output: number; trend: string; healthy: boolean }> = {};
    ...
    return stats as Record<ThermostatType, { output: number; trend: string; healthy: boolean }>;
  }
}
