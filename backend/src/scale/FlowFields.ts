...
export type FlowTarget = 'food' | 'water' | 'shelter' | 'community' | 'artifact' | 'custom';

export interface FlowFieldConfig {
  width: number;
  height: number;
  resolution: number;     // Células por flow vector (4 = 4x4 grid cells per vector)
  updateRate: number;     // Ticks entre actualizaciones
  diffuseStrength: number;// Suavizado del campo
}

const DEFAULT_CONFIG: FlowFieldConfig = {
  width: WORLD.WIDTH,
  height: WORLD.HEIGHT,
  resolution: 8,          // 8x8 grid cells = 1 flow vector
  updateRate: 10,
...
export class FlowField {
  ...
  // Dimensiones del grid de vectores
  private gridWidth: number;
  private gridHeight: number;
  
  // Arrays para dx, dy (Float32Array para eficiencia)
  private dx: Float32Array;
  private dy: Float32Array;
  
  // Distance field (para debug/visualización)
   ...
   */
  setTargets(targets: Array<{ x: number; y: number; strength?: number }>): void {
    // Reset distance field
    this.distance.fill(Infinity);
    
    // Marcar targets con distancia 0
  ...
  setFromField(field: Float32Array, threshold: number = 0.5): void {
    ...
    for (let gy = 0; gy < this.gridHeight; gy++) {
      for (let gx = 0; gx < this.gridWidth; gx++) {
        // Sample del centro de la celda del flow field
        ...
        if (worldX >= 0 && worldX < this.config.width && 
            worldY >= 0 && worldY < this.config.height) {
          const fieldIdx = Math.floor(worldY) * this.config.width + Math.floor(worldX);
  ...
  private computeDistances(): void {
    // BFS desde puntos con distancia 0
    const queue: number[] = [];
    
    // Encontrar todos los seeds
    ...
    // Direcciones: 8 vecinos (incluyendo diagonales)
    const dirs = [
      { dx: -1, dy: 0, cost: 1 },
      { dx: 1, dy: 0, cost: 1 },
      { dx: 0, dy: -1, cost: 1 },
  ...
  private computeGradients(): void {
    for (let y = 0; y < this.gridHeight; y++) {
      for (let x = 0; x < this.gridWidth; x++) {
        ...
        // Si es un target, no hay dirección
        ...
        // Calcular gradiente (diferencia con vecinos)
        const left = x > 0 ? this.distance[idx - 1] : Infinity;
        const right = x < this.gridWidth - 1 ? this.distance[idx + 1] : Infinity;
        const up = y > 0 ? this.distance[idx - this.gridWidth] : Infinity;
        const down = y < this.gridHeight - 1 ? this.distance[idx + this.gridWidth] : Infinity;
        
        // Gradiente: apunta hacia menor distancia
        let gdx = left - right;
        let gdy = up - down;
...
export class FlowFieldManager {
   ...
   */
  updateFromField(target: FlowTarget, scalarField: Float32Array, threshold: number = 0.5): void {
