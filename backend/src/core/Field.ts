...
export class Field {
  ...
  set(x: number, y: number, value: number): void {
    ...
    this.current[idx(x, y, this.width)] = Math.max(0, Math.min(this.config.maxValue, value));
  }
  ...
  add(x: number, y: number, delta: number): void {
    ...
    this.current[i] = Math.max(0, Math.min(this.config.maxValue, this.current[i] + delta));
  }
  ...
  growthStep(): void {
    ...
    for (let i = 0; i < this.size; i++) {
      ...
      // Crecimiento logístico: dv/dt = r * v * (1 - v/K)
  ...
  initWithNoise(baseValue: number, amplitude: number, seed: number): void {
    ...
    for (let i = 0; i < this.size; i++) {
      ...
      this.current[i] = Math.max(0, Math.min(this.config.maxValue, baseValue + noise));
    }
    
    // Suavizar con difusión
   ...
   */
  initWithOases(oases: Array<{ x: number; y: number; radius: number; value: number }>): void {
