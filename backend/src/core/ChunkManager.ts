...
export interface ChunkManagerConfig {
  worldWidth: number;
  worldHeight: number;
  activationRadius: number;  // Radio en chunks para activación
  hyperRadius: number;       // Radio en chunks para modo hyper
...
const DEFAULT_CHUNK_CONFIG: ChunkManagerConfig = {
  ...
  activationRadius: 3,
  hyperRadius: 1,
  sleepTimeout: 30000,  // 30 segundos
};
...
export class ChunkManager {
  ...
  // Centro de activación (donde está la cámara/acción)
  private focusX = 0;
  private focusY = 0;
  ...
  constructor(config: Partial<ChunkManagerConfig> = {}) {
    ...
    // Crear todos los chunks (inicialmente dormidos)
    ...
    console.log(`[ChunkManager] Created ${this.chunks.size} chunks (${this.chunksX}x${this.chunksY})`);
  }
   ...
   */
  worldToChunkLocal(worldX: number, worldY: number): { chunk: Chunk | undefined; localX: number; localY: number } {
    const cx = Math.floor(worldX / CHUNK_SIZE);
    const cy = Math.floor(worldY / CHUNK_SIZE);
   ...
   */
  addValue(type: FieldType, worldX: number, worldY: number, delta: number): void {
    const { chunk, localX, localY } = this.worldToChunkLocal(worldX, worldY);
    if (chunk && chunk.state !== 'dormant') {
  ...
  updateFromParticles(particles: Particle[]): void {
    // Encontrar bounding box de partículas
    ...
    // Activar chunks que contienen partículas
  ...
  private updateActivation(): void {
    ...
    // Calcular chunks que deben estar activos
    for (let dy = -activationRadius; dy <= activationRadius; dy++) {
      for (let dx = -activationRadius; dx <= activationRadius; dx++) {
        ...
        if (cx < 0 || cx >= this.chunksX || cy < 0 || cy >= this.chunksY) continue;
    ...
    // Dormir chunks que ya no están activos (después de timeout)
   ...
   */
  step(): void {
    // Solo actualizar chunks activos
    for (const key of this.activeChunks) {
      const chunk = this.chunks.get(key);
      if (!chunk || chunk.state === 'dormant') continue;
      
      // Hyper chunks se actualizan más
      if (chunk.state === 'hyper') {
        chunk.diffuseDecayStep();
        chunk.growthStep();
      } else {
        // Active chunks solo difusión
        chunk.diffuseDecayStep();
      }
    }
  }
   ...
   */
  getStats(): ChunkStats {
    let dormant = 0, active = 0, hyper = 0;
    
    for (const chunk of this.chunks.values()) {
      switch (chunk.state) {
        case 'dormant': dormant++; break;
        case 'active': active++; break;
        case 'hyper': hyper++; break;
      }
    }
   ...
   */
  initWithOases(type: FieldType, oases: Array<{ x: number; y: number; radius: number; value: number }>): void {
    for (const oasis of oases) {
      // Activar chunks afectados
      ...
      for (let cy = minCY; cy <= maxCY; cy++) {
        for (let cx = minCX; cx <= maxCX; cx++) {
          ...
          // Añadir oasis al campo del chunk
          const localOasis = {
          ...
          field.addOasis(localOasis.x, localOasis.y, localOasis.radius, localOasis.value);
        }
      }
    }
  }
