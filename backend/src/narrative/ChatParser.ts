...
export type Emotion = 'joy' | 'nostalgia' | 'love' | 'wonder' | 'melancholy' | 'neutral';
export type TimeOfDay = 'dawn' | 'day' | 'dusk' | 'night' | 'any';
export type ContextType = 'pair' | 'birth' | 'death' | 'community' | 'discovery' | 'conflict' | 'any';
...
export interface ChatFragment {
  ...
  trigger?: {
    field?: string;           // Campo semántico que activa
    threshold?: number;       // Umbral de activación
    time?: TimeOfDay;         // Momento del día
    context?: ContextType;    // Contexto de activación
  };
  discovered: boolean;        // Si ya fue descubierto
 ...
 */
export function detectEmotion(text: string): { emotion: Emotion; intensity: number } {
  const lower = text.toLowerCase();
  
  // Patrones de palabras clave
  const patterns: Record<Emotion, { words: string[]; baseIntensity: number }> = {
    joy: {
      words: ['jaja', 'haha', '😂', '😊', '🥰', 'feliz', 'happy', 'genial', 'increíble', 'wonderful', 'amazing', 'love', 'te amo', 'te quiero', '❤️', '💕'],
      baseIntensity: 0.7,
    },
    love: {
      words: ['te amo', 'i love you', 'mi amor', 'my love', 'beso', 'kiss', 'abrazo', 'hug', '❤️', '💕', '💗', 'heart', 'corazón', 'forever', 'siempre'],
      baseIntensity: 0.9,
    },
    nostalgia: {
      words: ['recuerdo', 'remember', 'antes', 'cuando', 'aquella vez', 'miss', 'extraño', 'ojalá', 'wish', 'tiempo', 'años'],
      baseIntensity: 0.6,
    },
    wonder: {
      words: ['wow', 'increíble', 'amazing', 'beautiful', 'hermoso', '✨', '🌟', 'mira', 'look', 'descubrí', 'found'],
      baseIntensity: 0.65,
    },
    melancholy: {
      words: ['triste', 'sad', 'difícil', 'hard', 'llorar', 'cry', 'dolor', 'pain', 'solo', 'alone', 'lejos', 'far', 'extraño', 'miss'],
      baseIntensity: 0.5,
    },
    neutral: {
      words: [],
  ...
  for (const [emotion, config] of Object.entries(patterns) as [Emotion, typeof patterns.joy][]) {
    let score = 0;
  ...
  // Ajustar intensidad por puntuación y mayúsculas
  if (text.includes('!')) intensity = Math.min(1, intensity + 0.1 * (text.match(/!/g)?.length || 0));
  if (text.includes('?')) intensity = Math.min(1, intensity + 0.05);
  if (text === text.toUpperCase() && text.length > 3) intensity = Math.min(1, intensity + 0.15);
...
function parseItem(item: any, id: number): ChatFragment | null {
  ...
  const speaker: Speaker = 
    item.speaker === 'isa' || item.sender === 'isa' || item.from === 'isa' ? 'isa' :
    item.speaker === 'stev' || item.sender === 'stev' || item.from === 'stev' ? 'stev' :
    item.is_from_me || item.isFromMe ? 'stev' : 'isa';
 ...
 */
function generateTrigger(emotion: Emotion, intensity: number): ChatFragment['trigger'] {
  ...
  return {
    ...triggers[emotion],
    threshold: 0.3 + (1 - intensity) * 0.5, // Más intensos requieren menos umbral
  };
}
...
export class ChatManager {
  ...
  getMatchingFragment(
    ...
    const candidates = this.fragments.filter(f => {
      ...
      if (context && f.trigger?.context && f.trigger.context !== 'any' && f.trigger.context !== context) return false;
      if (time && f.trigger?.time && f.trigger.time !== 'any' && f.trigger.time !== time) return false;
      return true;
    });
    
    if (candidates.length === 0) return null;
    
    // Priorizar por intensidad
    candidates.sort((a, b) => b.intensity - a.intensity);
    
    // Seleccionar con algo de aleatoriedad
  ...
  getStats(): ChatStats {
    const byEmotion: Record<Emotion, number> = {
      joy: 0, love: 0, nostalgia: 0, wonder: 0, melancholy: 0, neutral: 0,
    };
