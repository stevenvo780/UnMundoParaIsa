...
export interface BaseCharacter {
  ...
  // Historia
  bornAt: number;           // Tick de nacimiento
  age: number;              // Ticks vividos
  lineage: number[];        // IDs de ancestros
  events: CharacterEvent[]; // Eventos significativos
  
  // Relaciones
  partner?: number;         // ID de pareja
  children: number[];       // IDs de hijos
  community?: number;       // ID de comunidad
  
  // Personalidad (derivada de seed)
  traits: string[];
}
...
export interface Hero extends BaseCharacter {
  ...
  dialogues: string[];      // Diálogos disponibles
  achievements: string[];   // Logros
  legacy: string;           // Legado narrativo
  discoveredFragments: string[]; // IDs de fragmentos descubiertos
}

export interface CharacterEvent {
  tick: number;
  type: 'born' | 'paired' | 'child' | 'discovery' | 'migration' | 'conflict' | 'survival';
  description: string;
  location: { x: number; y: number };
}

// Nombres base para generación
const FIRST_NAMES = {
  male: ['Aiden', 'Kai', 'Leo', 'Milo', 'Nova', 'Orion', 'Phoenix', 'River', 'Sol', 'Zephyr'],
  female: ['Aurora', 'Celeste', 'Dawn', 'Ember', 'Flora', 'Iris', 'Luna', 'Sage', 'Stella', 'Willow'],
  neutral: ['Ash', 'Blake', 'Eden', 'Haven', 'Journey', 'Moss', 'Ocean', 'Rain', 'Sky', 'Storm'],
};

const FAMILY_SUFFIXES = ['del Valle', 'de la Luz', 'del Bosque', 'de las Estrellas', 'del Río', 'de la Luna'];

const TRAITS = [
  'curioso', 'valiente', 'gentil', 'sabio', 'persistente',
  'creativo', 'compasivo', 'aventurero', 'tranquilo', 'leal',
  'soñador', 'pragmático', 'alegre', 'reflexivo', 'protector',
];

const TITLES = [
  'El Explorador', 'La Visionaria', 'El Guardián', 'La Pionera',
  'El Fundador', 'La Matriarca', 'El Pacificador', 'La Sabia',
];
...
export function generateName(seed: number): string {
  ...
  // Determinar género basado en signature
  const genderIdx = Math.floor(sig[0] * 3);
  const genderKey = genderIdx === 0 ? 'male' : genderIdx === 1 ? 'female' : 'neutral';
...
export function generateTraits(seed: number, count: number = 3): string[] {
  ...
  // Usar signature para seleccionar traits únicos
  ...
  // Completar con traits aleatorios si faltan
...
export class MaterializationManager {
  ...
  // Configuración
  readonly config = {
    minAgeForCharacter: 500,      // Ticks para materializar a Character
    minAgeForHero: 2000,          // Ticks para promover a Hero
    minDiscoveries: 3,            // Descubrimientos para ser Hero
    materializationRadius: 50,    // Radio de cámara para materializar
  };
   ...
   */
  materialize(particle: Particle, age: number, lineage: number[] = []): Character {
  ...
  canPromoteToHero(character: Character): boolean {
    ...
    // Verificar logros
    const discoveries = character.events.filter(e => e.type === 'discovery').length;
  ...
  updateFromParticle(particle: Particle): void {
    ...
    const character = this.characters.get(characterId) || this.heroes.get(characterId);
    if (!character) return;
  ...
  handleDeath(particleId: number): void {
    ...
    // Mantener registro pero marcar como muerto
    const character = this.characters.get(characterId) || this.heroes.get(characterId);
    if (character) {
      character.events.push({
        tick: this.tick,
  ...
  getStats(): MaterializationStats {
    return {
      ...
      oldestCharacter: Math.max(0, ...Array.from(this.characters.values()).map(c => c.age)),
      oldestHero: Math.max(0, ...Array.from(this.heroes.values()).map(h => h.age)),
    };
  }
