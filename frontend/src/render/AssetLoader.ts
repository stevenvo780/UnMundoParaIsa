...
export class AssetLoader {
  ...
  private async doLoad(
    ...
    const loadedAssets: LoadedAssets = {
      terrain: {
        forest: ASSET_MANIFEST.terrain.forest.map(
          (p) => textures[p] as Texture,
        ),
        grassland: ASSET_MANIFEST.terrain.grassland.map(
          (p) => textures[p] as Texture,
        ),
      },
      trees: {
        forest: ASSET_MANIFEST.trees.forest.map((p) => textures[p] as Texture),
        grassland: ASSET_MANIFEST.trees.grassland.map(
          (p) => textures[p] as Texture,
        ),
      },
  ...
  getCharacterFrame(
    ...
    const characters = isFemale
      ? this.assets.characters.female
      : this.assets.characters.male;
  ...
  /**
   * Obtener tile de terreno basado en tipo de bioma
   * Mapea biomas especiales a assets disponibles
   */
  getTerrainTileByBiome(x: number, y: number, biome: string): Texture {
    if (!this.assets) throw new Error("Assets no cargados");
  ...
  getTreeTexture(x: number, y: number, isForest: boolean = true): Texture {
    ...
    const trees = isForest
      ? this.assets.trees.forest
      : this.assets.trees.grassland;
